...
function _coord_diff(a: coord, b: coord): coord {
  return {
    lat: a.lat - b.lat,
    lng: a.lng - b.lng,
  }
...
function __iteration__(fwd: coordOp, rev: coordOp) {
  return function (
    ...
    let diff = { lat: Infinity, lng: Infinity }

    let i = 0
    while (
      Math.max(Math.abs(diff.lat), Math.abs(diff.lng)) > tolerance &&
      i++ < 10
    ) {
